  ...
  return (
    <div className={cn('flex flex-col w-full mt-3',type ===1 ? 'items-start': 'items-end')}>
      <Card className={cn('w-[80%] p-0 border-0 shadow-none','flex gap-4 p-2', type===1 ? 'bg-white-400' :'bg-white-400')}>
        {type==1 && <div className=' bg-red-600 rounded-full w-16 h-16 text-white  flex-col items-center justify-center text-2xl hidden xl:block xl:flex'><RiRobot3Fill/></div>}
        ...
        {type==0 && <div className='bg-green-600 rounded-full w-16 h-16 text-2xl text-white flex-col items-center justify-center hidden xl:block xl:flex'><FaUser/></div>}
      </Card>
    </div>
  )
}
